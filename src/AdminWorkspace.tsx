...
function JobsiteForm({ initial, state, onSave, onClose }: { initial: ManagedJobsite | null; state: MasterDataState; onSave: (site: ManagedJobsite) => void; onClose: () => void }) {
...
function UserForm({ initial, state, onSave, onClose }: { initial: ManagedUser | null; state: MasterDataState; onSave: (user: ManagedUser) => void; onClose: () => void }) {
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    const form = new FormData(event.currentTarget);
    ...
    onSave({
      ...
      password: String(form.get("password")) || initial?.password || "demo123",
      displayName,
  ...
  return <AdminModal title={initial ? "แก้ไขผู้ใช้และสิทธิ์" : "เพิ่มผู้ใช้"} subtitle="กำหนดบทบาทและไซต์ที่เข้าถึงได้" onClose={onClose}><form className="admin-form" onSubmit={handleSubmit}><label>ชื่อผู้ใช้<input name="username" required pattern="[a-zA-Z0-9._-]{3,50}" defaultValue={initial?.username} /></label><label>ชื่อที่แสดง<input name="displayName" required defaultValue={initial?.displayName} /></label><label>รหัสผ่าน<input name="password" type="password" minLength={6} required={!initial} placeholder={initial ? "เว้นว่างเพื่อใช้รหัสเดิม" : "อย่างน้อย 6 ตัวอักษร"} /></label><label>บทบาท<select name="role" defaultValue={initial?.role ?? "user"}><option value="admin">ผู้ดูแลระบบ</option><option value="engineer">วิศวกร</option><option value="user">ผู้ใช้งาน</option></select></label><label className="wide">ตำแหน่ง / หน้าที่<input name="title" required defaultValue={initial?.title ?? "Customer Viewer"} /></label><fieldset className="wide"><legend>ไซต์ที่เข้าถึงได้</legend><div className="admin-site-checks">{state.jobsites.map((site) => <label key={site.id}><input name={`site:${site.id}`} type="checkbox" defaultChecked={initial?.jobsiteIds.includes(site.id) ?? false} /><span><strong>{site.name}</strong><small>{site.id}</small></span></label>)}</div></fieldset><label className="admin-check wide"><input name="active" type="checkbox" defaultChecked={initial?.active ?? true} /> เปิดใช้งานบัญชี</label><div className="admin-form-actions"><button type="button" onClick={onClose}>ยกเลิก</button><button className="primary" type="submit">บันทึกผู้ใช้</button></div></form></AdminModal>;
}

function AssetForm({ initial, state, onSave, onClose }: { initial: ManagedAsset | null; state: MasterDataState; onSave: (asset: ManagedAsset) => void; onClose: () => void }) {
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    const form = new FormData(event.currentTarget);
    onSave({ id: String(form.get("id")).trim().toUpperCase(), name: String(form.get("name")).trim(), type: String(form.get("type")).trim(), jobsiteId: String(form.get("jobsiteId")), location: String(form.get("location")).trim(), health: form.get("health") as AssetHealth, lastPm: String(form.get("lastPm")), nextPm: String(form.get("nextPm")), sensor: String(form.get("sensor")).trim(), active: form.get("active") === "on" });
  };
...
export default function AdminWorkspace({ currentUser, state, setState, onToast }: AdminProps) {
  ...
  const saveSite = (site: ManagedJobsite) => {
  ...
  const saveAsset = (asset: ManagedAsset) => {
  ...
  const removeSite = (site: ManagedJobsite) => {
    ...
    setState((current) => ({ ...current, jobsites: current.jobsites.filter((item) => item.id !== site.id) })); onToast(`ลบไซต์ ${site.id} แล้ว`);
  };
